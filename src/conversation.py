        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_SINGLE:
            ...
            for role, message in self.messages:
                if message:
                    ...
                    ret += role
            return ret
        elif self.sep_style == SeparatorStyle.PLAIN_CONCAT:
            ret = "" #self.system
            for i, (role, message) in enumerate(self.messages):
                if message:
                    if role == self.roles[0]:
                        if i >= len(self.messages) - 2: # last user input
                            ret += role + (f"[system]\n{self.system}\n\n" if self.system else "") + message + self.sep
                        else:
                            ret += role + message + self.sep
                    else:
...
if __name__ == "__main__":
    conv = get_conv_template("oaast_llama")
